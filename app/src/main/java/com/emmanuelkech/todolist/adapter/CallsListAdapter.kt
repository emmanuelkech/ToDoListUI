...
class CallsListAdapter : RecyclerView.Adapter<CallsListAdapter.ViewHolder>() {

    companion object {
        var taskTitle = arrayListOf(
            "Call the agent", "Conference call"
        )

        var taskDescription = arrayListOf(
            "remember to call the agent for the site allocation",
            "You have a conference call today"
        )
    }
        ...
        fun allViews(position: Int) {
            ...
            binding.taskHolder.setOnClickListener {
                ...
                Navigation.findNavController(binding.root)
                    .navigate(R.id.action_taskListFragment_to_callsDetailsFragment, bundle)
            }
        }
    }
}